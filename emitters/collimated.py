...
class CollimatedSource(LightSource):
    ...
    def __repr__(self):
        return f"<{self.type.capitalize()} light source. Intensity: {self.intensity}. Radius: {self.radius}. Attached = {self.attached}>"
    ...
    def export(self) -> TaichiSource:
        ...
        return TaichiSource(_type = COLLIMATED_SOURCE, intensity = vec3(self.intensity), 
                    pos = vec3(self.pos), dir = vec3(self.dir), r = self.radius, bool_bits = bool_bits)
